...
async function procesarAlertas() {
    ...
    for await (const line of rl) {
        ...
        if (isFirstLine) {
            headers = tds.map(h => h.replace(/<[^>]+>/g, '').trim());
    ...
    for (const [sucursal, alertasDeSucursal] of Object.entries(alertasPorSucursal)) {
        ...
        emailHtml += renderTable(criticos, 'CRITICO (HOY o VENCIDAS)', '#d32f2f');
        emailHtml += renderTable(advertencias, 'PROXIMAS 48 HORAS', '#f57f17');
        emailHtml += renderTable(proximos, 'PROXIMA SEMANA', '#388e3c');
        emailHtml += `</div>`;
